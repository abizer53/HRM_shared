...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('Create Employee'))
        {
            ...
            if($setings['employee_create'] == 1)
            {
                ...
                try
                {
                    $managers = User::where('created_by', \Auth::user()->creatorId())
                                ->whereHas(
                                    'roles', function($q){
                                        $q->where('name', 'hr');
                                    }
                    ...
                    foreach ($managers as $key => $manager) {
                        Mail::send([], [], function ($message) use($manager, $user) {
                        $message->to($manager->email)
                            ->subject('Info | New Employee Added.')
                            ->setBody('<h1>New Employee '.$user->name.' Added.</h1>', 'text/html'); 
                        });
                    }
                }
                catch(\Exception $e)
    ...
    public function show($id)
    {
        ...
        if(\Auth::user()->can('Show Employee'))
        {
            ...
            $employeesId  = \Auth::user()->employeeIdFormat($employee->employee_id);
            $leaves  = Leave::where(['employee_id' => $empId, 'status' => 'Approve'])->get();
