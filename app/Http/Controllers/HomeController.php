...
use App\Traits\ClockInOutTrait;
use App\Traits\LeaveCalculationTrait;

class HomeController extends Controller
{
    use ClockInOutTrait, LeaveCalculationTrait;
    ...
    public function index()
    {
        if(Auth::check())
        {
            ...
            if($user->type == 'employee')
            {
                ...
                $employeeIsLate = $this->employeeIsLate();

                $officeTime['startTime'] = Utility::getValByName('company_start_time');
                $officeTime['endTime']   = Utility::getValByName('company_end_time');

                $leaveData = [];
                $leaveTypesId = LeaveType::all()->pluck('id')->toArray();
                foreach ($leaveTypesId as $leaveId) {
                    $leaveData[] = $this->leaveCalculation($leaveId, auth()->user()->employee->id);   
                }

                return view('dashboard.dashboard', compact('arrEvents', 'announcements', 'employees', 'meetings', 'employeeAttendance', 'officeTime', 'settings', 'employeeIsLate', 'leaveData'));
            }
            else
            {
